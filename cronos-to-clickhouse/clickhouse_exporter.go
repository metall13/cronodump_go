...
func NewClickHouseExporter(config struct {
	...
	if err != nil {
		...
		if err := conn.Ping(ctx); err != nil {
			// Если ping не удался, переключаемся на SQL файл
			if verbose {
				fmt.Printf("Ошибка проверки подключения к ClickHouse: %v\n", err)
				fmt.Println("Переключаемся на режим SQL файла")
			}
			conn.Close()
			exporter.conn = nil
...
func (e *ClickHouseExporter) CreateTable(tableName string, fields []Field) error {
	...
	for _, field := range fields {
		...
		// Транслитерируем имя поля
		fieldName := e.transliterateFieldName(field.Name)
...
func (e *ClickHouseExporter) InsertBatch(tableName string, fields []Field, records []*Record) error {
	...
	for _, field := range fields {
		fieldName := e.transliterateFieldName(field.Name)
		columnNames = append(columnNames, fmt.Sprintf("`%s`", fieldName))
	}
...
func (e *ClickHouseExporter) transliterateFieldName(name string) string {
