...
// KODDecoder декодер для KOD кодирования Cronos
type KODDecoder struct {
	kod []byte
	inv []byte
}

// Исходная таблица KOD из cronodump
var initialKOD = []byte{
...
// NewKODDecoder создает новый KOD декодер
func NewKODDecoder() *KODDecoder {
	return NewKODDecoderWithTable(initialKOD)
}

// NewKODDecoderWithTable создает KOD декодер с пользовательской таблицей
func NewKODDecoderWithTable(kodTable []byte) *KODDecoder {
	decoder := &KODDecoder{
		kod: make([]byte, 256),
		inv: make([]byte, 256),
	}

	copy(decoder.kod, kodTable)

	// Вычисляем обратную таблицу
	for i, x := range decoder.kod {
		decoder.inv[x] = byte(i)
	}

	return decoder
}

// Decode декодирует данные с использованием KOD
// decode : shift, a[0]..a[n-1] -> b[0]..b[n-1]
//     b[i] = KOD[a[i]]- (i+shift)
func (kd *KODDecoder) Decode(offset int, data []byte) []byte {
	result := make([]byte, len(data))
	for i, b := range data {
		result[i] = byte((int(kd.kod[b]) - i - offset) % 256)
	}
	return result
}

// Encode кодирует данные с использованием KOD
// encode : shift, b[0]..b[n-1] -> a[0]..a[n-1]
//     a[i] = INV[b[i]+ (i+shift)]
func (kd *KODDecoder) Encode(offset int, data []byte) []byte {
	result := make([]byte, len(data))
	for i, b := range data {
		result[i] = kd.inv[(int(b)+i+offset)%256]
	}
	return result
}